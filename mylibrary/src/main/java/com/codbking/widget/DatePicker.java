...
class DatePicker extends BaseWheelPick {
    ...
    public void init() {
        ...
        datePicker.setLimitDate(isLimitDate);
     ...
     * @param hour
     */
    private void setChangeMinutSelect(int hour) {
        minutArr = datePicker.genMinut(hour);
        WheelGeneralAdapter adapter = (WheelGeneralAdapter) minuteView.getViewAdapter();
        adapter.setData(convertData(minuteView, minutArr));

        int indxt = datePicker.findIndextByValue(selectMinut, minutArr);
        if (indxt == -1) {
