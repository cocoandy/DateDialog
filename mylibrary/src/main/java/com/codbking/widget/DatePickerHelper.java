...
class DatePickerHelper {
    ...
    private void init() {
        ...
        Log.e("TAG_OO", YEAR_START + "->" + YEAR_START + "->" + MONTH_START + "->" + DAY_START + "->" + WEEK_START + "->" + HOUR_START + "->" + MINUTE_START);
    }
    ...
    public Integer[] genHour() {
        ...
        int startMinute = isLimitDate ? HOUR_START : 0;
        return genArr(24, startMinute, true);
    }

    public Integer[] genMinut() {
        Log.e("TAG_OO", "genMinut" + MINUTE_START);
        return genArr(60, MINUTE_START, true);
    }

    public Integer[] genMinut(int currHour) {
        Log.e("TAG_OO", "genMinut" + MINUTE_START);
        int startMinute = 0;
        if (isLimitDate && currHour == HOUR_START) {
            startMinute = MINUTE_START;
        }
        return genArr(60, startMinute, true);
    }

    public Integer[] genArr(int size, int start, boolean isZero) {
        tem.clear();
